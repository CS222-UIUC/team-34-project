...
def upvote_post(post_id):
    ...
    existing_upvote = Upvote.query.filter_by(user_id=current_user.id, post_id=post.id).first()
    ...
    return jsonify({"message": "Post upvoted successfully", "upvote_count": len(post.upvotes)}), 201
...
def upvote_reply(post_id, reply_id):
    ...
    existing_upvote = Upvote.query.filter_by(user_id=current_user.id, reply_id=reply.id).first()
    ...
    return jsonify({"message": "Reply upvoted successfully", "upvote_count": len(reply.upvotes)}), 201
